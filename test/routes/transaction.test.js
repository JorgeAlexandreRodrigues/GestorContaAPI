...
beforeAll(async () => {
  ...
  const users = await app.db('users').insert([
    { name: 'User #1', email: 'user@mail.com', password: '$2a$10$MkjKP6teUXb922SG9YksqO5ZqGdEYfwe85n74NgkqVxYwhp.gKi62' },
    { name: 'User #2', email: 'user2@mail.com', password: '$2a$10$MkjKP6teUXb922SG9YksqO5ZqGdEYfwe85n74NgkqVxYwhp.gKi62' },
  ], '*');
  [user, user2] = users;
...
// Aula 62 Deve ajustar o sinal das transações
...
// Aula 64 Reusar Código
describe(' Ao tentar inserir uma transação válida', () => {
  // const validTransaction = { description: 'New T', date: new Date(),
  // ammount: 100, type: 'I', acc_id: accUser.id };

  // let validTransaction;
  // beforeAll(() => {
  //   validTransaction = { description: 'New T', date: new Date(), ammount: 100,
  // type: 'I', acc_id: accUser.id };
  // });
  ...
  // Aula 63 Deve validar a inserção da transação
  test('Não deve inserir uma transação sem descrição', () => testTemplate({ description: null }, 'Descriçao e um atributo obrigatorio'));
  test('Não deve inserir uma transação sem valor', () => testTemplate({ ammount: null }, 'Valor e um atributo obrigatorio'));
  test('Não deve inserir uma transação sem data', () => testTemplate({ date: null }, 'Data e um atributo obrigatorio'));
  test('Não deve inserir uma transação sem conta', () => testTemplate({ acc_id: null }, 'Conta e um atributo obrigatorio'));
  test('Não deve inserir uma transação sem tipo', () => testTemplate({ type: null }, 'Tipo e um atributo obrigatorio'));
  test('Não deve inserir uma transação com tipo invalido', () => testTemplate({ type: 'A' }, 'Tipo inválido'));
});

test('Devo retorna transação por ID', () => {
  return app.db('transactions').insert(
...
// Aula 58 devo inserir trans e depois depois o send com o update
...
// Aula 61
test('Não deve remover transação de outro usuário', () => {
  return app.db('transactions').insert(
...
// Aula 65 devemos colocar uma tran e depois tentar remover a conta
// ir ao serviço da conta colocar a condição if
test('Não deve remover uma conta com transação', () => {
  return app.db('transactions').insert(
    ...
    .then(() => request(app).delete(`/v1/accounts/${accUser.id}`)
      ...
      .then((res) => {
        expect(res.status).toBe(400);
        expect(res.body.error).toBe('Essa conta possui transaçoes associadas');
      }));
});
